 ...
 * @Package:        com.flylo.frame.tool.log
 * @ClassName:      LoggerPrinter
 * @Author:         ANWEN
 * @CreateDate:     2020/6/27 12:53 PM
 * @UpdateUser:
 * @UpdateDate:     2020/6/27 12:53 PM
 * @UpdateRemark:
 * @Version:
 */
internal class LoggerPrinter : Printer {
    ...
    override fun map(map: Map<*, *>) {
        if (map != null) {
            ...
            for (entry in map.entries) {
                ...
                stringBuilder.append((entry as Map.Entry<*, *>).value)
    ...
    private fun logContent(priority: Int, chunk: String) {
        val lines =
            chunk.split(LINE_SEPARATOR!!.toRegex()).toTypedArray()
        for (line in lines) {
            logChunk(
                priority,
                "$HORIZONTAL_DOUBLE_LINE $line"
            )
        }
    }
    ...
    private fun getStackInfo(priority: Int) {
        logChunk(
            priority,
            "$HORIZONTAL_DOUBLE_LINE[Thread] → " + Thread.currentThread()
                .name
        )
    ...
    companion object {
        ...
        private const val TOP_BORDER =
            TOP_LEFT_CORNER.toString() + DOUBLE_DIVIDER
        private const val BOTTOM_BORDER =
            BOTTOM_LEFT_CORNER.toString() + DOUBLE_DIVIDER
        private const val MIDDLE_BORDER =
            MIDDLE_CORNER.toString() + SINGLE_DIVIDER
